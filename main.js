$(document).ready(function(){
	...
	*/
	...
	$("body").delegate("#product","click",function(event){
		...
		$.ajax({
			url : "action.php",
			method : "POST",
			data : {addToFavorite:1,proId:pid},
			success : function(data){
				count_item();
				getFavoriteItem();
				$('#product_msg').html(data);
	...
	//Add Product into favorite End Here
	...
	//Count user favorite items funtion
	favorite_item();
	function favorite_item(){
		$.ajax({
			url : "action.php",
			method : "POST",
			data : {favorite_item:1},
			success : function(data){
				$(".badge").html(data);
			}
		})
	}
	//Count user favorite items funtion end
	...
	//Fetch Cart item from Database to dropdown menu
	//Fetch Favorite item from Database to dropdown menu
	getFavoriteItem();
	function getFavoriteItem(){
		$.ajax({
			url : "action.php",
			method : "POST",
			data : {Common:1,getFavoriteItem:1},
			success : function(data){
				$("#favorite_product").html(data);
			}
		})
	}
	...
	*/
	...
	$("body").delegate(".remove","click",function(event){
		...
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{removeItemFromFavorite:1,rid:remove_id},
			success	:	function(data){
				$("#favorite_msg").html(data);
				checkOutDetails();
			}
		})
	})
	...
	*/
	...
	$("body").delegate(".update","click",function(event){
		...
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{updateFavoriteItem:1,update_id:update_id,qty:qty},
			success	:	function(data){
				$("#favorite_msg").html(data);
				checkOutDetails();
			}
	...
	*/
	...
	function checkOutDetailsFavorite(){
	 $('.overlay').show();
		$.ajax({
			url : "action.php",
			method : "POST",
			data : {Common:1,checkOutDetailsFavorite:1},
			success : function(data){
				$('.overlay').hide();
				$("#favorite_checkout").html(data);
					net_total();
			}
		})
	}
	...
	*/
	...
	/*
		net_total function is used to calcuate total amount of favorite item
	*/
	function net_total_favorite(){
		var net_total_favorite = 0;
		$('.qty').each(function(){
			var row = $(this).parent().parent();
			var price  = row.find('.price').val();
			var total_favorite = price * $(this).val()-0;
			row.find('.total_favorite').val(total_favorite);
		})
		$('.total_favorite').each(function(){
			net_total += ($(this).val()-0);
		})
		$('.net_total_favorite').html("Total : Lei " +net_total_favorite);
	}
